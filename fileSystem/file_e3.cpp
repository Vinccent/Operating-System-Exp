...
void write_handler(string filename) {
    bool flag = false;
    for (auto it = cur->files.begin(); it != cur->files.end(); it++) {
        fileNode* tmpfile = *it;
        if (tmpfile->name == filename) {
...
void read_handler(string filename) {
    bool flag = false;
    for (auto it = cur->files.begin(); it != cur->files.end(); it++) {
        fileNode* tmpfile = *it;
        if (tmpfile->name == filename) {
...
void handle(string cmd) {
    ...
    if (cmd.substr(0, 5) == "write") {
        string filename = cmd.substr(6, cmd.size() - 6);
        write_handler(filename);
    }
    if (cmd.substr(0, 4) == "read") {
        string filename = cmd.substr(5, cmd.size() - 5);
        read_handler(filename);
    }
}
